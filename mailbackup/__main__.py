...
def main():
    ...
    def on_interrupt(signum, frame):
        ...
        logger.debug("Executors interrupted, saving manifest queue...")
        # Save manifest state
        manifest.dump_queue()
        logger.debug("Manifest queue saved, exiting...")
        # Stop status thread
        status_thread.stop()
        sys.exit(1)
    ...
    start = time.time()
    logger.debug("Startup complete, parsing input and executing pipeline...")
